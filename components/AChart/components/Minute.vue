<template>
	<view class="minute">
		<view class="textBox">
			<view class="title">均价: 
				<template v-if="moveShowData">
					{{toPlusFixed(moveShowData[2], 2)}}
				</template>
			</view>
			<view class="sub" :class="[$setClass(moveShowData[1], yestClose)]">
				最新: 
				<template v-if="moveShowData">
					{{toPlusFixed(moveShowData[1], 2)}} {{toPlusFixed(moveShowData[1] - yestClose, 2)}}
				</template>
			</view>
		</view>
