<template>
	<view class="kline">
		<template v-if="!klineIsLoading">
			<view class="moveDataBox">
		...
		<template v-if="klineIsLoading">
		    <view class="klineLoad" :style="{height: height[0] + 'rpx'}">
		        <Loading></Loading>
		    </view>
		</template>
	</view>
...
<script>
	...
	export default {
		...
		data() {
			return {
				...
				klineParams: { // K线请求参数
					...
					mkt_type: 2,    // 复权类型 1-前复权，2-不复权（除权），3-后复权
					div_type: 1,
				}, 
				volMax: 0,      // 成交量最大值
		...
		mounted() {
			...
			this.klineParams.date_type = typeObj[type];
		...
		methods: {
			...
			equityTickerKline(){
				...
				let { klineParams, xAxisLen } = this;
				this.$api.equityTickerKline(klineParams).then(res => {
			...
			draw({ctx, volCtx, data}){
				...
				let { issuePrice, upColor, downColor, xScale, volYScale, volMax, lineMax, lineMin, lineYScale, pixelRatio, bgLineColor, scaleTextColor, timeHeight, xAxisLen } = this;
				// 绘制背景网格
				...
				data.map((item, index) => {
					...
					let openPrice      = item[1],
						closePrice     = item[2],
						lastClosePrice = data[index - 1] ? data[index - 1][2] : issuePrice;
